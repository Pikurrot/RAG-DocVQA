...
class HiVT5(T5ForConditionalGeneration):
	...
	def _prepare_encoder_decoder_kwargs_for_generation(
		...
		"""
		Prepare the kwargs for the forward pass of the decoder in generation mode.
		:param inputs_tensor: The input tensor to the model.
		:param model_kwargs: The model kwargs.
		:param model_input_name: The name of the input tensor to the model.
		...
		for p_idx in range(max(model_kwargs['num_pages'])):
			...
			if model_kwargs.get('output_attentions', False):
		...
		model_kwargs["encoder_outputs"] = ModelOutput(output_model_kwargs)
		return model_kwargs
	...
	def prepare_inputs_for_generation(
		...
		cross_attn_head_mask: Optional[torch.LongTensor] = None,
	...
	def forward(
		...
		num_pages: Optional[List[int]] = None,
		answer_page_idx: Optional[torch.LongTensor] = None,
		use_cache: Optional[bool] = None,
		output_attentions: Optional[bool] = None,
		output_hidden_states: Optional[bool] = None,
		...
		if encoder_outputs is None:
			...
			page_encoder_attentions = encoder_outputs.get('attentions', None) if output_attentions else None
...
class Proxy_HiVT5:
	...
	def to(self, device: Any):
		...
		self.model.spatial_embeddings.to(device)
		self.model.visual_embeddings.to(device)
		self.model.retrieval_module.to(device)
	...
	def prepare_vqa_input_ids(self, batch: dict):
	...
	def get_answer_from_model_output(
		...
		output = self.model.generate(
			...
			output_scores=True  # Ensure logits are returned
		)
	...
	def forward_record_and_retrieve_attention_dict(self, record: dict):
	...
	def inference(
		...
		return self.forward(batch, output_attentions, return_pred_answer)
