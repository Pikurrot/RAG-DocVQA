...
def is_float_equal(reference, prediction, include_percentage: bool = False, is_close: float = False) -> bool:
    """Check if two float values are equal, with optional percentage handling."""
...
def is_exact_match(s):
    """Check if string requires exact matching."""
    flag = False
    # Website
    if "https://" in s:
        flag = True
...
def eval_score(gt, pred, answer_type):
    """Evaluate score based on answer type."""
    if answer_type == "Int":
        try:
...
def show_results(samples, show_path=None):
    ...
    with open(show_path, 'w') as f:
        ...
        # Single-page vs Cross-page analysis
        ...
        # Evidence sources and document types analysis
